...
const components = fs.readdirSync(path.resolve(__dirname, '../packages/components/'))
...
components.forEach(name => {
  externals[`components/${name}`] = `lib/${name}`
})
...
exports.components = components.reduce((last, curr) => {
    console.log(curr,'components...')
  last[`components/${curr}`] = path.resolve(__dirname, `../packages/components/${curr}`)
  return last
}, {})
